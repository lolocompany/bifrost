...
func TestAppendSourceHeaders(t *testing.T) {
	...
	got := append(append([]kgo.RecordHeader{}, bridge.AppendSourceHeaders(base, id, r)...), r.Headers...)

	if len(got) != 5 {
		t.Fatalf("headers: len %d want 5", len(got))
	}
	if string(got[0].Key) != bridge.HeaderSourceCluster || string(got[0].Value) != "east" {
		t.Fatalf("cluster header: %+v", got[0])
	}
	if string(got[1].Key) != bridge.HeaderSourceTopic || string(got[1].Value) != "in" {
		t.Fatalf("topic header: %+v", got[1])
	}
	if string(got[2].Key) != bridge.HeaderSourcePartition {
		t.Fatalf("partition key: %s", got[2].Key)
	}
	if binary.BigEndian.Uint32(got[2].Value) != 3 {
		t.Fatalf("partition value: %v", got[2].Value)
	}
	if string(got[3].Key) != bridge.HeaderSourceOffset {
