...
func TestRunWithClients_retriesAfterPollError(t *testing.T) {
	...
	err := relay.RunWithClients(ctx, testIdentity(), consumer, producer, m, testOptions(relay.Options{}))
...
func TestRunWithClients_returnsWrongTopicError(t *testing.T) {
	...
	err := relay.RunWithClients(context.Background(), testIdentity(), consumer, producer, m, testOptions(relay.Options{}))
...
func TestRunWithClients_retriesProduceErrorWithBackoff(t *testing.T) {
	...
	err := relay.RunWithClients(ctx, testIdentity(), consumer, producer, m, testOptions(relay.Options{
		Retry: relay.RetryPolicy{
			Produce: relay.RetryConfig{MinBackoff: time.Second, MaxBackoff: time.Second},
		},
...
func TestRunWithClients_retriesCommitErrorInPlace(t *testing.T) {
	...
	err := relay.RunWithClients(ctx, testIdentity(), consumer, producer, m, testOptions(relay.Options{
		Retry: relay.RetryPolicy{
			Commit: relay.RetryConfig{MinBackoff: 2 * time.Second, MaxBackoff: 2 * time.Second},
		},
		Sleep: func(_ context.Context, d time.Duration) error {
			sleeps = append(sleeps, d)
			return nil
...
func TestRunWithClients_commitCoalescingByThreshold(t *testing.T) {
	...
	producer := &fakeProducer{}
	err := relay.RunWithClients(ctx, testIdentity(), consumer, producer, m, relay.Options{
		BatchSize:          1,
...
func (f *fakeConsumer) PollFetches(context.Context) relay.FetchResult {
	f.pollCalls++
	if f.pollDelay > 0 {
...
func (m *testMetrics) IncMessages(relay.Identity) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.messages++
}

func (m *testMetrics) IncErrors(_ relay.Identity, stage string) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if m.errors == nil {
...
func (m *testMetrics) ObserveProduceDuration(_ relay.Identity, seconds float64) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.produceDurations = append(m.produceDurations, seconds)
}

func (m *testMetrics) AddConsumerSeconds(_ relay.Identity, state string, seconds float64) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *testMetrics) AddProducerSeconds(_ relay.Identity, state string, seconds float64) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func testIdentity() relay.Identity {
...
func testOptions(opts relay.Options) relay.Options {
