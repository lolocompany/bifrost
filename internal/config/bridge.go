...
type Bridge struct {
	...
	// Replicas is relay goroutines for this relay. Use 0 or omit for automatic sizing from the
	// source topic partition count (after topic ensure), subject to process-wide heuristics in
	// internal/app. If set (>0), that many goroutines run; each uses the same to-side producer and
...
// EffectiveConsumerGroup returns the consumer group for this relay.
