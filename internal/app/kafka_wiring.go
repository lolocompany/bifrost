...
// RelayOptionsFromBridge builds relay.Options from validated bridge and cluster config.
func RelayOptionsFromBridge(periodicStatsInterval time.Duration, bridgeCfg config.Bridge, fromCluster, toCluster config.Cluster) (relay.Options, error) {
	...
	if s := strings.TrimSpace(bridgeCfg.CommitInterval); s != "" {
		...
		if err != nil {
			return relay.Options{}, fmt.Errorf("relay.commit_interval: %w", err)
		}
		commitInterval = d
	}
